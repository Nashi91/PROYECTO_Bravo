...
-- Se hace una comprobación de si el personaje existe en dicha tabla.
	...
	ELSE
--Si el personaje no existe en la tabla se crea otra donde se insertan los personajes muertos.
...
-- En el caso de existir un rey o legado, con esto comprobamos de que existe tal rey o legado. Y hacemos su correspondiente eliminación.
	...
	PRINT 'ERROR UN REY O LEGADO EXISTE :V'
	ELSE
	BEGIN
		UPDATE REINOS
		SET REY = NULL
		WHERE REINOS.REY = (SELECT USUARIO FROM DELETED)
	END
	BEGIN
		UPDATE REINOS
		SET LEGADO = NULL
...
AS
	UPDATE TERRITORIOS
	SET REINO = (SELECT REINOS.NOMBRE FROM REINOS WHERE REINOS.NOMBRE = @KINGDOM_DEAD)
	WHERE TERRITORIOS.NOMBRE IS NULL
...
AS
	...
	BEGIN
		IF (SELECT FECHA_FIN FROM INSERTED)>
			(SELECT FECHA_INICIO FROM EVENTOS DATEDIFF(DAY,FECHA_FIN,FECHA_INICIO) WHERE EVENTOS.CODEVNT = (SELECT CODEVNT FROM INSERTED) --Comprobar que la fecha FIN es superior a la de inicio respetando un día como mínimo
		UPDATE EVENTOS
